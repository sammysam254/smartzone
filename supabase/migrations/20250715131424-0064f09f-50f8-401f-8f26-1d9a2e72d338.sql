...
DROP POLICY IF EXISTS "Admins can confirm mpesa payments" ON public.mpesa_payments;
CREATE POLICY "Admins can confirm mpesa payments" ON public.mpesa_payments 
FOR UPDATE 
USING (
  EXISTS (
...
DROP POLICY IF EXISTS "Admins can update all order statuses" ON public.orders;
CREATE POLICY "Admins can update all order statuses" ON public.orders 
FOR UPDATE 
USING (
  EXISTS (
    SELECT 1 FROM public.profiles 
